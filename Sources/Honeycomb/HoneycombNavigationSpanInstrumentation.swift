...
internal class HoneycombNavigationProcessor {
    ...
    func reportNavigation(path: NavigationPath, reason: String? = nil) {
    ...
    func reportNavigation(path: String, reason: String? = nil) {
        reportNavigation(path: [path], reason: reason)
    }

    func reportNavigation(path: Encodable, reason: String? = nil) {
        do {
    ...
    func reportNavigation(path: [Encodable], reason: String? = nil) {
        do {
            ...
            reportNavigation(path: pathStack, reason: reason)
        } catch {
            reportNavigation(path: unencodablePath, reason: reason)
        }
    }

    func reportNavigation(path: Any, reason: String? = nil) {
        reportNavigation(path: unencodablePath, reason: reason)
    }
...
extension View {
    @available(iOS 16.0, macOS 13.0, *)
    public func instrumentNavigation(path: NavigationPath, reason: String? = nil) -> some View {
        HoneycombNavigationProcessor.shared.reportNavigation(path: path, reason: reason)

        return modifier(EmptyModifier())
    }

    public func instrumentNavigation(path: Encodable, reason: String? = nil) -> some View {
        HoneycombNavigationProcessor.shared.reportNavigation(path: path, reason: reason)
